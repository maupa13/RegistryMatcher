...
public class RegistryMatcher {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Введите путь к excel файлу (.xls) "
                + "с форматом файла (например - c:\\input.xls):");
        String inputFile = scanner.next();

        System.out.println("Введите путь c наименованием и форматом excel файла (.xls) "
                + "для выдачи результата проверки (например - c:\\output.xls):");
        String outputFile = scanner.next();

        System.out.println("УВЕДОМЛЕНИЕ: Закройте таблицу отправляемую на проверку."
                + " При ошибке программа перезапустится самостоятельно.");
        ...
        try {
            ...
            for (int i = 1; i < rowcount + 1; i++) {
                ...
                if (nonNull(cellOgrn)) {
                    ...
                    String urlZakachicki = "https://zakupki.gov.ru/epz/customer223/search/"
                            ...
                            + "NAME&customer223Status_0=on&customer223Status"
                            + "=0&organizationRoleValueIdNameHidden=%7B%7D";
                    ...
                    System.out.println("\"Реестр заказчиков\": " + cellOgrn.getStringCellValue()
                            + ". Выполнено: " + (100 * i / rowcount) + "%");
            ...
            for (int n = 1; n < rowcount + 1; n++) {
                ...
                if (nonNull(cellOgrn1)) {
                    ...
                    Matcher matcherClasses = Pattern.compile("registry-entry__header-top__title")
                    ...
                    all = organizaciiStatus.toString()
                            .contains("44-ФЗ") && organizaciiStatus.toString().contains("223-ФЗ");
                    only44 = organizaciiStatus.toString()
                            .contains("44-ФЗ") && !(organizaciiStatus.toString().contains("223-ФЗ"));
                    only223 = !(organizaciiStatus.toString()
                            .contains("44-ФЗ")) && organizaciiStatus.toString().contains("223-ФЗ");
                    blocked44 = organizaciiStatus.toString()
                            .contains("44-ФЗ Заблокирована");
                    blocked223 = organizaciiStatus.toString()
                    ...
                    if (blocked223) {
                        dataCell1.setCellValue("Запись о блокировке: 223-ФЗ. "
                                + "Рекомендуется проверить организацию вручную");
                    }

                    if (blocked44) {
                        dataCell1.setCellValue("Запись о блокировке: 44-ФЗ. "
                                + "Рекомендуется проверить организацию вручную");
                    }

                    if (countOfReference == 1) {
                        if (all && !(blocked44) && !(blocked223)) {
                            dataCell1.setCellValue("Зарегистрирован: 223-ФЗ и 44-ФЗ. "
                                    + "Записи о дочерних организациях отсутствуют");
                        } else if (only223 && !(blocked223)) {
                            dataCell1.setCellValue("Зарегистрирован: 223-ФЗ. "
                                    + "Записи о дочерних организациях отсутствуют");
                        } else if (only44 && !(blocked44)) {
                            dataCell1.setCellValue("Зарегистрирован: 44-ФЗ. "
                                    + "Записи о дочерних организациях отсутствуют");
                        }
                    } else if (countOfReference > 0) {
                        if (all && !(blocked44) && !(blocked223)) {
                            dataCell1.setCellValue("Материнская компания зарегистрирована: "
                                    + "223-ФЗ и 44-ФЗ");
                        } else if (only223 && !(blocked223)) {
                            dataCell1.setCellValue("Зарегистрирован: 223-ФЗ. "
                    ...
                    System.out.println("\"Реестр организаций\": " + cellOgrn1.getStringCellValue()
                            + ". Выполнено: " + (100 * n / rowcount) + "%");
            ...
            for (int l = 1; l < rowcount + 1; l++) {
                ...
                if (nonNull(cellName)) {
                    ...
                    String urlVyruchka = "https://zakupki.gov.ru/epz/revenue/search/results.html?searchString="
                            ...
                            + "reportingPeriodYearEndHidden=0&reportingPeriod"
                            + "QuarterEndHidden=DEFAULT&sortBy=REESTR_NAME"
                    ...
                    Matcher matcherClassesVyruchka = Pattern.compile("2022 год")
                    ...
                    System.out.println("\"Реестр сведений о выручке\": " + cellName.getStringCellValue()
                            + ". Выполнено: " + (100 * l / rowcount) + "%");
                    ...
                    System.out.println("\"Реестр сведений о выручке\". "
                            + "Пустые ячейки в excel файле.");
                }
            }
